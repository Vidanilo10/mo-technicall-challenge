...
def get_payment_body():
    return {
        "external_id_character": "external_01-01-01",
        "total_amount": 1000.0,
        "paid_at": "2019-08-24T14:15:22Z",
        "status": 1,
        "customer": "external_03",
        "loan_external_id": "external_01-01"
    }
...
def test_list_payments(create_superuser, get_client, get_token, get_url):
    create_superuser
    client = get_client
    response = client.get(f"{get_url}api/v1/payment/", headers={"Authorization": f"Token {get_token}"})
    assert response.status_code == 200


@pytest.mark.django_db
def test_create_payment(create_superuser, create_customer, create_loan, get_client, get_token, get_url, get_payment_body):
    create_superuser
    create_customer
    create_loan
    client = get_client
    ...
    assert response_content.get("external_id_character") == "external_01-01-01"


@pytest.mark.django_db
def test_get_payments_by_customer(create_superuser, get_client, get_token, create_customer, get_url):
    create_superuser
    customer_external_id = create_customer.get("external_id_character")
    client = get_client
    response = client.get(f"{get_url}api/v1/payment/?customer={customer_external_id}", headers={"Authorization": f"Token {get_token}"})
    assert response.status_code == 200
